...
def badWorker(a_tid,a_account):
    global g_mutex
    print "Bad Str", a_tid, datetime.datetime.now()
    for i in range(500000):
        g_mutex.acquire()
        a_account.withdraw(1);
        g_mutex.release()
    print "Bad End " , a_tid , datetime.datetime.now()  
...
if __name__ == "__main__": 
    ...
    for i in range(10): 
        if i<=5:
            ...
            th = threading.Thread(target=badWorker,args=(i,acc))
